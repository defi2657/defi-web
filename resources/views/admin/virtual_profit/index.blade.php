...
@section('scripts')
    <script>
        layui.use(['table','form','layer','laydate'], function(){
            ...
            table.render({
                elem: '#demo'
                ,url: '{{url('admin/virtual_profit/list')}}' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
            ...
            $('#add_financial_machine').click(function(){
                ...
                var index = layer.open({
                    title:'添加虚拟数据'
                    ,type:2
                    ,content: '/admin/virtual_profit/add'
                    ,area: ['800px', '600px']
                    ,maxmin: true
                    ,anim: 3
                });
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('真的删除行么', function(index){
                        $.ajax({
                            url:'{{url('admin/virtual_profit/del')}}',
                            type:'post',
                            dataType:'json',
                            data:{id:data.id},
                    ...
                    layer_show('编辑','{{url('admin/virtual_profit/add')}}?id='+data.id,1200,500);
                }
            });
            //监听提交
