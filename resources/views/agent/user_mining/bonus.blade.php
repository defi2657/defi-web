...
@section('scripts')
    <script>
        layui.use(['table','form','layer','laydate','element'], function(){
            ...
            table.render({
                elem: '#demo'
                ,url: '{{url('agent/user_mining/bonusList')}}' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
            ...
            {{--    var type = $('#type').val();--}}
            {{--    tbRend("{{url('/agent/user_mining/bonusList')}}?account_number="+account+'&type='+type+'&start_time='+start_time+'&end_time='+end_time);--}}
            {{--    return false;--}}
            {{--});--}}
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    ...
                    layer_show('编辑矿机','{{url('agent/mining_machine/add')}}?id='+data.id);
                }
            });
            //监听提交
