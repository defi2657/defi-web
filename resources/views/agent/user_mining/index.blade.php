...
@section('scripts')
    <script>
        layui.use(['table','form','layer','laydate'], function(){
            ...
            table.render({
                elem: '#demo'
                ,url: '{{url('agent/user_mining/list')}}' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
                    {field: 'id', title: 'ID', Width:60, sort: true}
                    ,{field: 'user_id', title: '用户ID', Width:80}
                    ,{field: 'account_number', title: '用户名', Width:80}
                    // ,{field: 'mining_name', title: '矿机名字', Width:80}
            ...
            $('#add_mining_machine').click(function(){
                // layer_show('添加管理员', '/agent/mining_machine/add');
                var index = layer.open({
                    title:'添加套餐'
                    ,type:2
                    ,content: '/agent/mining_machine/add'
                    ,area: ['800px', '600px']
                    ,maxmin: true
                    ,anim: 3
                });
            ...
            form.on('switch(is_up)', function(obj){
                ...
                console.log(id);
                $.ajax({
                    url:'{{url('agent/mining_machine/up')}}',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
            ...
            form.on('switch(is_newuser)', function(obj){
                ...
                console.log(id);
                $.ajax({
                    url:'{{url('agent/mining_machine/newuser')}}',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
                    success:function (res) {
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('真的删除行么', function(index){
                        $.ajax({
                            url:'{{url('agent/mining_machine/del')}}',
                            type:'post',
                            dataType:'json',
                            data:{id:data.id},
                            success:function (res) {
                    ...
                    layer_show('矿机分红详情','{{url('agent/user_minig/bonusdetail')}}?id='+data.id,1200,500);
                }else if(obj.event==='expiration'){
                    layer.confirm('真的提前到期么？收益会立刻打入用户账户', function(index){
                        $.ajax({
                            url:'{{url('agent/user_mining/acceleration')}}',
                            type:'get',
                            dataType:'json',
                            data:{id:data.id},
                            success:function (res) {
