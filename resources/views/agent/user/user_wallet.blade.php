    ...
    <script>
        window.onload = function() {
            ...
            layui.use(['element', 'form', 'layer', 'table'], function () {
                ...
                tbRend("{{url('/agent/user/walletList')}}?user_id=" + user_id);
                ...
                form.on('switch(sexDemo)', function(obj){
                    ...
                    $.ajax({
                        url:'{{url('agent/user/wallet_lock')}}',
                        type:'post',
                        dataType:'json',
                        data:{id:id},
                ...
                table.on('tool(userlist)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if (layEvent === 'conf') { 
                        ...
                        var index = layer.open({
                            title: '管理提币地址'
                            , type: 2
                            , content: '{{url('/agent/user/address')}}?id=' + data.id
                            , maxmin: true
                        });
                        layer.full(index);
                        ...
                        layer.confirm('链上归拢首先要确保钱包内有手续费,确定要开始归拢?', function (index) {
                            ...
                            layer.close(index);
                            $.ajax({
                                url: '{{url('agent/chain/balance_collect')}}',
                                type: 'post',
                                dataType: 'json',
                                data: {id: data.id},
                                success: function (res) {
