...
use App\Models\MiningReturnsBonus;
...
class UserMiningController extends Controller
{
    ...
    public function index(){
        return view("agent.user_mining.index");
    }
    public function bonusIndex(){
        return view("agent.user_mining.bonus");
    }
    public function bonusDetail(Request $request){
        $id = $request->get('id', null);
        return view("agent.user_mining.bonusdetail",['id' => $id]);
    }
    public function mining_user_bonus(){
        return view("agent.user_mining.mining_user_bonus");
    }
    public function invite_user(Request $request){
        $id = $request->get('id', null);
        return view("agent.user_mining.invite",['id'=>$id]);
    }
    ...
    public function userBonusList(Request $request){
        ...
        $list=MiningReturnsBonus::where('user_mining_id',$id)->paginate($limit);
        return $this->layuiData($list);
    }
    ...
    public function acceleration(Request $request){
        ...
        //已分红数量
        $aready_bonus_num=MiningReturnsBonus::where('user_mining_id',$id)->sum('num');
        //本金数量
        $user_mining=UserMining::find($id);
        $capital_num=$user_mining->num;
        //已获得邀请分红数量
        ...
        //一次性给还本金和分红
        $bonus_num=bcsub($user_mining->bonus_num,$aready_bonus_num,8);
        if($bonus_num>0) {
            DB::beginTransaction();
            try {
                $invite_num = bcmul($bonus_num, $parent_bonus_rate, 8);
                $user_wallet = UsersWallet::where('user_id', $user_mining->user_id)
                    ->lockForUpdate()
                    ->where('currency', 3)
                    ->first();//USDT:3
                if (!$user_wallet) {
                ...
                $user_mining->is_return = 1;
                $user_mining->return_time = time();
                $user_mining->status = -1;
                $user_mining->remark = "提前到期";
                $user_mining->save();

                $mining_returns_bonus = new MiningReturnsBonus();//分红
                $mining_returns_bonus->date = $date;
                $mining_returns_bonus->user_id = $user_mining->user_id;
                $mining_returns_bonus->user_mining_id = $id;
                $mining_returns_bonus->num = $bonus_num;
                $mining_returns_bonus->price = 1;
                $mining_returns_bonus->total = $user_mining->num;
                ...
                $mining_returns_bonus = new MiningReturnsBonus();//本金
                $mining_returns_bonus->date = $date;
                $mining_returns_bonus->user_id = $user_mining->user_id;
                $mining_returns_bonus->user_mining_id = $id;
                $mining_returns_bonus->num = $user_mining->num;
                $mining_returns_bonus->price = 1;
                $mining_returns_bonus->total = $user_mining->num;
                $mining_returns_bonus->rate = 1;
                $mining_returns_bonus->addtime = time();
                ...
                $mining_returns_bonus->save();
                $result = change_wallet_balance($user_wallet, 2, $user_mining->num, AccountLog::MINING_CAPITAL, '矿机本金退还-提前退还');
                if ($result !== true) {
                    throw new \Exception($result);
                }

                $user = Users::getById($user_mining->user_id);
                if ($user->parent_id != 0) {
    ...
    public function miningUserBonusList(Request $request)
    {
        ...
        foreach ($list->items() as $item){
            ...
            $buy_mining_num=UserMining::where('user_id',$item['id']);
            if(!empty($start_time)){
                $buy_mining_num=$buy_mining_num->where('create_time','>=',strtotime($start_time));
            }
            if(!empty($end_time)){
                $buy_mining_num=$buy_mining_num->where('create_time','<=',strtotime($end_time));
            }
            $buy_mining_num=$buy_mining_num->sum('num');
            ...
            $child_mining_num=0;
            if(count($idArr)>0) {
                $child_mining_num = UserMining::whereIn('user_id', $idArr);
                if(!empty($start_time)){
                    $child_mining_num=$child_mining_num->where('create_time','>=',strtotime($start_time));
                }
                if(!empty($end_time)){
                    $child_mining_num=$child_mining_num->where('create_time','<=',strtotime($end_time));
                }
                $child_mining_num=$child_mining_num->sum('num');
            }
...
//            }
            $team_mining_num=0;
            if(count($idArr)){
                $team_mining_num = UserMining::whereIn('user_id', $idArr);
                if(!empty($start_time)){
                    $team_mining_num=$team_mining_num->where('create_time','>=',strtotime($start_time));
                }
                if(!empty($end_time)){
                    $team_mining_num=$team_mining_num->where('create_time','<=',strtotime($end_time));
                }
                $team_mining_num=$team_mining_num->sum('num');
            }
            array_push($res, array(
                'id' => $item['id'],
                'phone' => $item['phone'],
                'email' => $item['email'],
                'buy_mining_num' => $buy_mining_num,//购买产品金额
                'child_mining_num'=>$child_mining_num,//直推总业绩
                'team_mining_num'=>$team_mining_num
            ));
        }
    ...
    public function invite_user_list(Request $request){
        ...
        $res=array();
        foreach ($list->items() as $item){
            $buy_mining_num=UserMining::where('user_id',$item['id'])->sum('num');
            array_push($res, array(
                'id' => $item['id'],
                'phone' => $item['phone'],
                'email' => $item['email'],
