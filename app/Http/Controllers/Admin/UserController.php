...
class UserController extends Controller
{
    ...
    public function doedit()
    {
        ...
        $is_agent=Input::get("is_agent",'');
        $agent_rate=Input::get("agent_rate",'');
        ...
        try {
            ...
            if($is_agent=='on'){
                if($agent_rate=='' || $agent_rate<=0||$agent_rate>1){
                    return $this->error("代理比例设置错误");
                }
                $user->is_agent=1;
            }else{
                $user->is_agent=0;
            ...
            $agent_rate!=''&& $user->agent_rate=$agent_rate;
            $user->save();
            // 收款信息
            $cashinfo = UserCashInfo::unguarded(function () use ($id) {
                return UserCashInfo::firstOrNew(['user_id' => $id]);
