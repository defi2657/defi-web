...
class AccountLog extends Model
{
    ...
    public static $trade_scene=[
        '0' => '无', '1' => '奖金池', '2' => '奖金', '3' => '游戏仓库'
    ];

    const TRADE_SCENE_NONE=0;
    const TRADE_SCENE_LEGAL=1;
    const TRADE_SCENE_BALANCE=2;
    const TRADE_SCENE_LEVER_BALANCE=3;
    ...
    const WAllET_DIVIDENT_IN=17;//分红划入
    ...
    const MINING_BALANCE_OUT=308;//矿机购买扣除
    const MINING_BONUS=309;//矿机分红
    const MINING_PARENT_BONUS=310;//矿机邀请分红
    const MINING_CAPITAL=311;//矿机本金


    const FINANCIAL_BALANCE_OUT=312;//理财购买扣除
    const FINANCIAL_BONUS=313;//理财分红
    const FINANCIAL_PARENT_BONUS=314;//理财邀请分红
    const FINANCIAL_CAPITAL=315;//理财本金    
    const FINANCIAL_REDEEM=316;//赎回
    const FINANCIAL_WALLET_OUT=317;//理财提币

    const FINANCIAL_EXCHANGE=318;//提取奖励
    const FINANCIAL_EXCHANGE_ADD_CHANGE=319;//提取奖励增加钱包2金额
    const FINANCIAL_TOKENEXCHANGE=320;//领取奖励扣除token钱包
    const FINANCIAL_TOKENEXCHANGE_ADD_LEGAL=321;//领取奖励增加钱包1金额
    const FINANCIAL_AGENT_BONUS=322;//代理分红
    const Enter_LOCK_LEVER_BALANCE=400;
    const Exit_LOCK_LEVER_BALANCE=401;
    ...
    public static function getTypeInfo($type)
    {
        switch ($type) {
            ...
            case self::ADMIN_CANDY_BALANCE:
                return '后台调节通证';
                break;
            case self::FINANCIAL_WALLET_OUT:
                return '理财提币';
                break;
            return '暂无此类型';
            break;
